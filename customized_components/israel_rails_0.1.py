...
def setup(hass, config):
    ...
    url = 'https://rail.co.il/apiinfo/api/Plan/GetRoutes?OId={}&TId={}&Date={}&Hour=2400&isGoing=true'.format(fromStation, toStation, Tdate)
    ...
    while(TimeNow + datetime.timedelta(hours=Tgmt) - nextTrain).total_seconds() >= 0:
        i=i+1
        nextTrain = data["Data"]["Routes"][i]["Train"][0]["DepartureTime"]
        nextTrain = datetime.datetime.strptime(nextTrain , "%d/%m/%Y %H:%M:%S")
